...
def vehicle_feasibility_check(vehicle_plan, vehicle_index, problem):
    ...
    """
    if not vehicle_plan or len(vehicle_plan) == 0:
...
def zero_pos(sol):
    ...
    zero_indices = (sol_array == 0).nonzero()[0]
...
"""
def soft_greedy_reinsert_one_vehicle(calls, prob, removed_sol):
...
This reinsertion function chooses a random vehicle, then finds the best position to place the pickup and delivery for all calls
"""
def soft_greedy_reinsert(calls, prob, removed_sol, feasibility_cache, cost_cache):
    best_sol = removed_sol
...
def weighted_removal(prob, sol, reinsert, feasibility_cache, cost_cache): 
    ...
    if calls_to_reinsert:
        ...
        new_sol, new_cost = reinsert(selected_calls, prob, new_sol, feasibility_cache, cost_cache)
...
def random_removal_1(prob, sol, reinsert, feasibility_cache, cost_cache, boolean):
    ...
    new_sol, new_cost = reinsert(calls_to_reinsert, prob, new_sol, feasibility_cache, cost_cache)
...
def random_removal_2(prob, sol, reinsert, feasibility_cache, cost_cache):
    ...
    if not calls_list:
        print("Eror: No calls in the chosen vehicle.")
        return new_sol
    if len(calls_list) < 10:
        calls_n = np.random.randint(1, len(calls_list) + 1)
    else:
...
def OP5(prob, sol, feasibility_cache, cost_cache):
    new_sol, new_cost = random_removal_1(prob, sol, soft_greedy_reinsert, feasibility_cache, cost_cache, True)

    return new_sol, new_cost

def OP6(prob, sol, feasibility_cache, cost_cache):
    new_sol, new_cost = random_removal_1(prob, sol, k_regret, feasibility_cache, cost_cache, True)
...
def acceptance_probability(new_sol, incumbent, incumbent_cost, i, total_iterations, best_cost, new_cost, delta_E, operator):
    g = i
    G = total_iterations
    ...
    if delta_E < 0:
        ...
        incumbent = new_sol.copy()
        incumbent_cost = new_cost
        # print(f"Accepted new solution: {new_cost}")
...
def general_adaptive_metaheuristics(prob, initial_sol, plot_results = True): 
    ...
    for i in range(total_iterations):
        ...
        if i % 1000 == 0:
            print(f"********Iteration {i}, Best Cost: {best_cost}********, Best solution: {best_sol}") 
        
        if i_since_best != 0 and i_since_best % 500 == 0:
            incumbent, incumbent_cost, best_sol, best_cost, i_since_best = escape_algorithm(prob, incumbent, incumbent_cost, best_sol, best_cost, i_since_best, feasibility_cache, cost_cache)
